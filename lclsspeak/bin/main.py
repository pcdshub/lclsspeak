"""
`lclsspeak` is the top-level command for accessing various subcommands.
...
def _try_import(module):
    ...
    return importlib.import_module(relative_module, 'lclsspeak.bin')
...
def _build_commands():
    ...
    if unavailable:
        ...
        for module, ex in unavailable:
            DESCRIPTION += (
                f'\nWARNING: "lclsspeak {module}" is unavailable due to:'
...
def main():
    ...
    top_parser.add_argument(
        '--version', '-V',
        action='version',
        version=lclsspeak.__version__,
        help="Show the lclsspeak version number and exit."
    )
    ...
    logger = logging.getLogger('lclsspeak')
